...
#if defined(NDEBUG)
# error "eUK cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf49ac5ff3fdb1343dd5e4b5b13d5f7f2170e343afaa101d4b04d04c8ff252054");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eUK: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eUK Signed Message:\n";
...
{
    // eUK: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eUK: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eUK: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1457366767, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "UK to send ships to tackle people smugglers";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049dbda10e8decc309d91e8cf46387a4d2fe434f9934da81c5d138cabbadc51de68b9468ec87e31efe21dfdd786278ae5c74a5e730f2bd9e4f44c6826acff94daf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1457366767;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1441340;
        ...
        assert(block.hashMerkleRoot == uint256("0x3a7e3373d94887f865d6644e215eeb097db8a47b3a885c9fe7a6ee9a811443b1"));
...
unsigned char pchMessageStart[4] = { 0xd5, 0x28, 0xf8, 0xc3 }; // eUK: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eUKMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eUKMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eUKMiner(CWallet *pwallet)
{
    printf("eUKMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("euk-miner");
    ...
    try { loop {
        ...
        printf("Running eUKMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eUKMiner, pwallet));
}
